    ...
    <nav className="w-screen flex justify-center">
      <div className="flex p-[15px] fixed z-10 top-[50px] w-[70vw] bg-yellow/[.7] rounded-[50px] backdrop-blur-[20px] backdrop-saturate-[1.7]	">
        <div className="w-6/12 md:w-2/12 xl:w-2/12">
          <a href={`/${language}`+"#"+t("header.links.home")}>
        ...
        <div className="hidden md:flex md:w-10/12 xl:w-10/12 flex-row justify-end gap-[20px]">
          <div className="flex justify-end">
            <a className="flex items-center" href={"#"+t("header.links.about_me")}>
              <p className="font-paragraph uppercase text-[15px]">{t("header.about_me")}</p></a>
          </div>
          <div className="flex justify-end">
            <a className="flex items-center" href={"#"+t("header.links.projects")}>
              <p className="font-paragraph uppercase text-[15px]">{t("header.projects")}</p>
            </a>
          </div>
          <div className="flex justify-end">
            <a className="flex items-center" href={"#"+t("header.links.contact")}>
      ...
      <div className={`block md:hidden flex-col md:flex-row ${isOpen ? '' : 'hidden'} fixed top-0 left-0 bg-yellow w-full h-full z-50 pt-0 p-[50px]`}>
        ...
        <div className="md:hidden flex flex-col mt-[100px]">
          <a href={`/${language}`+"#"+t("header.links.about_me")} className="text-black font-paragraph uppercase font-regular text-[20px] mr-4 mb-[20px]" onClick={toggleMenu}>{t("header.about_me")}</a>
          <a href={`/${language}`+"#"+t("header.links.projects")} className="text-black font-paragraph uppercase font-regular text-[20px] mr-4 mb-[20px]" onClick={toggleMenu}>{t("header.projects")}</a>
          <a href={`/${language}`+"#"+t("header.links.contact")} className="text-black font-paragraph uppercase font-regular text-[20px] mr-4 mb-[20px]" onClick={toggleMenu}>{t("header.contact")}</a>
        </div>
        <div>
          <LanguageSelector/>
        </div>
